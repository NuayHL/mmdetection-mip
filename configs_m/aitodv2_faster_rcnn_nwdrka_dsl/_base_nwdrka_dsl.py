"""NWD-RKA + DSL-DYAB combined base config.

Applies both enhancements to the AITOD-v2 Faster R-CNN baseline:

  **RPN stage** (from NWD-RKA):
    * RankingAssigner with the NWD metric — for each GT, assigns the top-k
      proposals ranked by Normalized Wasserstein Distance as positives.
    * Standard AnchorGenerator is kept (NWD-RKA does not change anchors).
...
NWD-RKA (RPN assignment) and DSL-DYAB (RCNN assignment) are orthogonal:
the former changes which proposals RPN treats as positive, the latter
changes how the second-stage head is supervised. All other settings
(dataset, schedule, backbone, neck, test config) are inherited from the
``aitodv2_faster_rcnn/_base_.py`` baseline.
"""
