...
class YOLOX_test2_l_refine_soft_Head(BaseDenseHead):
    ...
    def init_weights(self) -> None:
        ...
        super(YOLOX_test2_l_refine_soft_Head, self).init_weights()
    ...
    def forward(self, x: Tuple[Tensor]):
        ...
        gate_value, topk_idx, distributes = self.get_topk_and_distribute(gate_value)
        ...
        return raw_result, (gate_value, topk_idx, distributes)
    ...
    def predict_by_feat(self,
        ...
        gate_value, topk_idx, distributes = gate_value_and_topk_idx
        batch_lenth = len(batch_img_metas)
        result_list = [None] * batch_lenth
        for i in range(self.num_experts):
            distribute_idx = distributes[i]
            # if no data sent to this expert
            if distribute_idx is None:
                continue
            task_idx = distribute_idx[0]
    ...
    def loss_by_feat(self,
        ...
        loss_dict['loss_gate_obj'] = self.loss_gate_obj_loss(shrink_gate_value, selected_obj_loss)
    ...
    def get_topk_and_distribute(self, gate_value):
        ...
        distributes = self.distribute_to_experts_with_weight(top_gate_value, topk_idx)
        ...
        return fin_gate_value, topk_idx, distributes
    ...
    def distribute_to_experts_with_weight(self, top_softmax, topk_idx):
        """distribute task within a batch to different experts, with its correspond logist value
            ...
            return:
                List[ [distribute_idx, correspond_logists], [ ... ], ... ]
        ...
        for expert_id in range(self.num_experts):
            ...
            if data_indices.numel() > 0:
                ...
                if self.training:
                    ...
                    output[expert_id] = [data_indices, torch.zeros((), device=topk_idx.device)]
        return output
    ...
    def _loss_by_feat(
        ...
        Args:
            ...
            objectnesses (Sequence[Tensor]): Score factor for
                all scale level, each is a 4D-tensor, has shape
                (batch_size, 1, H, W).
            weight: the weight for each batch_idx, if is [0,], it means this experts
                is not distribute with data, thus ignored.
        ...
        # set the fake distributed loss as zero
        if num_imgs == 1 and weight == 0:
            loss_obj_label = loss_obj_label * weight
