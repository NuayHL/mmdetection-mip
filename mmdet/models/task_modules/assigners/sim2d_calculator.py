...
from mmdet.structures.bbox import bbox_overlaps, get_box_tensor, bbox_overlaps_ext
...
class BboxSiM2D:
    ...
    def __init__(self, scale=1., dtype=None, mode=None, iou_kwargs={}):
        self.scale = scale
        self.dtype = dtype
        self.mode = mode
    ...
    def __call__(self, bboxes1, bboxes2, mode='sim', is_aligned=False):
        ...
        if mode in ['diou', 'ciou', 'd_interpiou', 'interpiou', 'hausdorff']:
        ...
        if self.dtype == 'fp16':
            ...
            overlaps = bbox_metric(bboxes1, bboxes2, mode, is_aligned, **self.iou_kwargs)
            if not overlaps.is_cuda and overlaps.dtype == torch.float16:
                # resume cpu float32
                overlaps = overlaps.float()
            return overlaps

        return bbox_metric(bboxes1, bboxes2, mode, is_aligned, **self.iou_kwargs)

    def __repr__(self):
        """str: a string describing the module"""
        repr_str = self.__class__.__name__ + f'(' \
            f'scale={self.scale}, dtype={self.dtype}, \
            mode={self.mode}, iou_kwargs={self.iou_kwargs})'
        return repr_str
